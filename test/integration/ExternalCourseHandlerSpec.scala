  ...
  "ExternalCourseHandler" when:
    "getting courses with default organisation" should:
      "return course with external grade scale" in:
        ...
        val result = get("/app/courses?filter=code&q=2121219", session = session)
        status(result).must(be(Status.OK))

        val coursesJson = contentAsJson(result).as[JsArray]
      ...
      "return course with organisation from external data" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          val result = get("/app/courses?filter=code&q=MAT21014", session = session)
          status(result).must(be(Status.OK))
    ...
    "updating courses" should:
      "update existing course with new data" in:
        loginAsAdmin().map { case (user, session) =>
          ...
          // Import a new course
          courseInfoServlet.setFile(new File("test/resources/courseUnitInfo.json"))
          get("/app/courses?filter=code&q=2121219", session = session)
    ...
    "searching for remote courses" should:
      "always search remote even when local course exists" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          courseInfoServlet.setFile(new File("test/resources/courseUnitInfo.json"))
          val result = get("/app/courses?filter=code&q=2121219_abcdefghijklmnop", session = session)
          status(result).must(be(Status.OK))
      ...
      "get course from another organisation" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          val result = get("/app/courses?filter=code&q=t7", session = session)
          status(result).must(be(Status.OK))
    ...
    "handling authorization" should:
      ...
      "deny access to unauthenticated users" in:
        ...
        val result = get("/app/courses?filter=code&q=2121219")
