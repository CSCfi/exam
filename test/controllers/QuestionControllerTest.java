...
public class QuestionControllerTest extends IntegrationTestCase {
    ...
    public void testAddEssayQuestionToExam() {
        ...
        JsonNode draft = Json
            ...
            .set("questionOwners", Json.newArray().add(Json.newObject().put("id", userId)));
    ...
    private void deleteAddedOption(Question question) {
        List<MultipleChoiceOption> options = question
            .getOptions()
            .stream()
        ...
        ((ObjectNode) json).set("questionOwners", Json.newArray().add(Json.newObject().put("id", userId)));
    ...
    @NotNull
    private Question addNewOption(
        Question question,
        Double defaultScore,
        Double[] expectedDefaultScores,
        double minDefaultScore
    ) {
        List<MultipleChoiceOption> options = question
            .getOptions()
            .stream()
        ...
        JsonNode json = Json
            ...
            .set(
                ...
                Json
                    ...
                    .add(Json.newObject().put("defaultScore", defaultScore).put("option", "Uusi"))
            );
        ((ObjectNode) json).set("questionOwners", Json.newArray().add(Json.newObject().put("id", userId)));
        Result result = request(Helpers.PUT, "/app/questions/" + question.getId(), json);
        assertThat(result.status()).isEqualTo(200);
        ...
        List<Double> defaultScores = saved
            .getOptions()
            .stream()
    ...
    public void testClaimChoiceQuestionCreateAndUpdate() {
        ...
        ArrayNode options = Json.newArray().add(correctOption).add(incorrectOption).add(skipOption);
        ...
        ArrayNode modifiedOptions = Json.newArray().add(correctOptionUpdated).add(incorrectOption).add(skipOption);

        JsonNode updatedQuestion = createClaimChoiceQuestionJson(
    ...
    JsonNode createClaimChoiceQuestionJson(String question, ArrayNode options) {
        JsonNode draft = Json
            ...
            .set("options", options);
        ((ObjectNode) draft).set("questionOwners", Json.newArray().add(Json.newObject().put("id", userId)));
        return draft;
    }
}
