...
class QuestionControllerSpec extends BaseIntegrationSpec with DbApiHelper:
  ...
  private lazy val question: Question = {
    DB
      .find(classOf[Question])
      .list
      .find(q =>
        q.getQuestion.contains("Kumpi vai kampi") && q.getType == Question.Type.WeightedMultipleChoiceQuestion
      ) match
      case Some(q) =>
        q.getOptions.size must be(3)
        q.getQuestionOwners.add(DB.find(classOf[User], 5L)) // Use a known test user ID
        q.update()
        q
      case None => fail("No question found")
  }
  ...
  "QuestionController" when:
    "adding essay question to exam" should:
      "create, update and add question successfully" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          // Create draft
          val createResult = makeRequest(POST, "/app/questions", Some(draft), session = session)
          status(createResult).must(be(Status.OK))
          val createJson      = contentAsJson(createResult)
          ...
          val updateResult = put(s"/app/questions/$questionId", update, session = session)
          status(updateResult).must(be(Status.OK))
          val updateJson        = contentAsJson(updateResult)
          val updatedQuestionId = (updateJson \ "id").as[Long]

          // Add to exam
          val addToExamData = Json.obj("sequenceNumber" -> 0)
          val addResult = makeRequest(
            POST,
            s"/app/exams/$examId/sections/$sectionId/questions/$updatedQuestionId",
            Some(addToExamData),
            session = session
          )
          status(addResult).must(be(Status.OK))
          val sectionJson      = contentAsJson(addResult)
    ...
    "working with weighted multiple choice questions" should:
      "add option to weighted multiple choice question" in:
        loginAsTeacher().map { case (user, session) =>
          assertExamSectionQuestion(question, 3, 4.0, Array(2.0, 2.0, -2.0), -2.0)

          // Add new option to question
          val updatedQuestion = addNewOption(question, Some(0.75), Array(1.0, 1.0, -1.0, 0.75), -1.0, user, session)
      ...
      "add null score option to weighted multiple choice question" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          // Add new option to question
          val updatedQuestion = addNewOption(question, None, Array(1.0, 1.0, -1.0, Double.NaN), -1.0, user, session)
      ...
      "add negative option to weighted multiple choice question" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          // Add new option to question
          val updatedQuestion = addNewOption(question, Some(-0.73), Array(1.0, 1.0, -1.0, -0.73), -1.73, user, session)
      ...
      "delete option from weighted multiple choice question" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          val questionWithNewOption =
            addNewOption(question, Some(0.75), Array(1.0, 1.0, -1.0, 0.75), -1.0, user, session)
      ...
      "delete negative option from weighted multiple choice question" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          assertExamSectionQuestion(question, 3, 4.0, Array(2.0, 2.0, -2.0), -2.0)
          val questionWithNewOption =
            addNewOption(question, Some(-0.5), Array(1.0, 1.0, -1.0, -0.5), -1.5, user, session)
    ...
    "exporting questions" should:
      "export questions to Moodle format" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          val result = makeRequest(POST, "/app/questions/export", Some(params), session = session)
    ...
    "working with claim choice questions" should:
      "create and update claim choice question" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          val updateResult = put(s"/app/questions/${saved.getId}", updatedQuestion, session = session)
          status(updateResult).must(be(Status.OK))
      ...
      "reject empty claim choice question options" in:
        loginAsTeacher().map { case (user, session) =>
          ...
          val result = makeRequest(POST, "/app/questions", Some(draft), session = session)
  ...
  private def deleteAddedOption(question: Question, user: User, session: play.api.mvc.Session): Unit =
    ...
    val result = put(s"/app/questions/${question.getId}", json, session = session)
    status(result).must(be(Status.OK))
  ...
  private def parseQuestionFromResponse(result: concurrent.Future[play.api.mvc.Result]): Question =
