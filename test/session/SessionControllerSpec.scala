  ...
  "SessionController" when:
    ...
    "handling login for a user with permissions" must:
      "include permissions in session and response body" in:
        ...
        val teacher = DB.find(classOf[User]).where().eq("eppn", "teacher@funet.fi").find
          .getOrElse(fail("teacher@funet.fi not found in test data"))

        val perm = new Permission
        perm.`type` = PermissionType.CAN_INSPECT_LANGUAGE
        perm.save()
        teacher.permissions.add(perm)
        teacher.update()

        val (_, session) = runIO(loginAsTeacher())
        session.data.get("permissions") mustBe Some("CAN_INSPECT_LANGUAGE")
