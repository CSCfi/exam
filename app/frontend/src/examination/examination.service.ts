...
export class ExaminationService {
    ...
    startExam$(hash, isPreview, isCollaboration, id): Observable<Examination> {
        ...
        return this.http.get<Examination>(isCollaboration ?
            url.replace('/app/', '/integration/iop/') : url).pipe(
                tap(e => {
                    if (e.cloned) {
    ...
    saveTextualAnswer = (esq: ExaminationQuestion, hash, autosave): Observable<ExaminationQuestion> => {
        ...
        const url = this.getResource(type === 'EssayQuestion' ?
    ...
    private isTextualAnswer = (esq: ExaminationQuestion) => {
    ...
    saveAllTextualAnswersOfSection = (section: ExaminationSection, hash, autosave):
        Observable<ExaminationQuestion[]> => {

        const questions = section.sectionQuestions.filter(esq => this.isTextualAnswer(esq));
