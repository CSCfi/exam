    ...
    controller: class SectionsListComponentController implements ng.IComponentController {
        ...
        moveSection = (section: ExamSection, from: number, to: number) => {
            if (from >= 0 && to >= 0 && from !== to) {
                this.$http
                    .put(this.Exam.getResource(`/app/exams/${this.exam.id}/reorder`, this.collaborative), {
                        from: from,
                        to: to,
                    })
                    .then(() => {
        ...
        addNewSection = () => {
            this.$http
                .post(this.Exam.getResource(`/app/exams/${this.exam.id}/sections`, this.collaborative), {})
