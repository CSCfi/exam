...
class ReviewDocumentsController @Inject()(cc: ControllerComponents,
      ...
      .andThen(authorized(Seq(Role.Name.TEACHER, Role.Name.ADMIN))) { request =>
        ...
        body.file("file") match {
          case Some(file) =>
            ...
            val role = if (user.hasRole(Role.Name.ADMIN)) Role.Name.ADMIN else Role.Name.TEACHER
  ...
  def getArchivedAttachments(eid: Long,
                             start: Option[String],
                             end: Option[String]): Action[AnyContent] =
  ...
  private def isEligibleForArchiving(exam: Exam, start: Option[DateTime], end: Option[DateTime]) =
    exam.hasState(Exam.State.ABORTED, Exam.State.REVIEW, Exam.State.REVIEW_STARTED) &&
      !(start.isDefined && exam.getCreated.isBefore(start.get)) && !(end.isDefined && exam.getCreated
      .isAfter(end.get))

  private def createArchive(prototype: Exam,
                            aos: ArchiveOutputStream,
                            start: Option[DateTime],
                            end: Option[DateTime]): Unit = {
  ...
  private def createSummaryFile(aos: ArchiveOutputStream,
                                start: Option[DateTime],
                                end: Option[DateTime],
                                exam: Exam,
                                questions: Map[Long, String]): Unit = {
