...
class ExternalExamExpirationActor @Inject (
  ...
  private def processExternalExam(ee: ExternalExam): Future[Unit] =
    ...
    if attachments.nonEmpty then
      ...
      logger.info(s"Processing $count attachments for external exam ${ee.getId} with max concurrency of $maxConcurrency")
      Source(attachments)
  ...
  override def createReceive(): AbstractActor.Receive = receiveBuilder()
    .`match`(
      ...
      (_: String) =>
        ...
        val externalExams = DB.find(classOf[ExternalExam])
          .where
          .isNotNull("sent")
