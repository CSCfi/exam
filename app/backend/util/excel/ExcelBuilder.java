...
public class ExcelBuilder {
  ...
  private static List<Tuple2<String, CellType>> getScoreReportDefaultCells(
    ...
    scoreReportCells.add(Tuple.of(examScore.isPresent() ? examScore.get().getId().toString() : "", CellType.STRING));
  ...
  public static ByteArrayOutputStream buildScoreExcel(Long examId, Collection<Long> childIds) throws IOException {
    ...
    childExams.forEach(
      exam -> {
        ...
        Map<Long, Tuple2<String, CellType>> scoreCellsByQuestionIds = exam
          .getExamSections()
          .stream()
          .flatMap(es -> es.getSectionQuestions().stream())
          .collect(
    ...
    /* Autosize cells */
    IntStream
      .range(0, questionIds.size() + ScoreReportDefaultHeaders.length)
