...
public class ExaminationRepository {
    ...
    public CompletionStage<Optional<Exam>> getPossibleClone(String hash, User user, CollaborativeExam ce, PathProperties pp) {
        return CompletableFuture.supplyAsync(() -> {
            ExpressionList<Exam> query = createQuery(pp).where()
                    .eq("hash", hash)
                    .eq("creator", user);
            if (ce == null) {
