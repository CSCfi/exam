...
public class SessionController extends BaseController {
  ...
  private CompletionStage<Result> handleExternalReservationAndCreateSession(
    ...
    if (reservation != null) {
      try {
        return handleExternalReservation(user, reservation).thenApplyAsync(r -> createSession(user, true, request));
  ...
  private void updateUser(User user, Http.Request request) throws AddressException {
    user.setOrganisation(parse(request.header("homeOrganisation").orElse("")).map(this::findOrganisation).orElse(null));
