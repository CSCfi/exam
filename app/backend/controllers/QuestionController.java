...
public class QuestionController extends BaseController implements SectionQuestionHandler {
  ...
  public Result getQuestions(
    ...
    if (user.hasRole(Role.Name.ADMIN) && Stream.of(examIds, courseIds, tagIds, sectionIds).allMatch(List::isEmpty)) {
  ...
  private void createOption(Question question, JsonNode node, User user) {
    ...
    option.setClaimChoiceType(
      SanitizingHelper.parseEnum("claimChoiceType", node, MultipleChoiceOption.ClaimChoiceOptionType.class).orElse(null)
    );
  ...
  public Result exportQuestions(Http.Request request) {
    ...
    Set<Long> ids = StreamSupport.stream(node.spliterator(), false).map(JsonNode::asLong).collect(Collectors.toSet());
