...
public class CollaborativeReviewController extends CollaborationController {
            ...
            .thenApplyAsync(
              response -> {
                ...
                while (it.hasNext()) {
                  ...
                  if (!node.path("user").path("eppn").asText().equals(eppn) || node.path("_id").asText().equals(aid)) {
                    it.remove();
                  }
                }
  ...
  private Optional<CompletionStage<Result>> validateExamState(Exam exam, boolean gradeRequired, User user) {
    ...
    if (
      exam.hasState(Exam.State.ABORTED, Exam.State.GRADED_LOGGED, Exam.State.ARCHIVED) || exam.getExamRecord() != null
    ) {
