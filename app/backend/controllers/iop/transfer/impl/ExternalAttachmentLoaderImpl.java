...
public class ExternalAttachmentLoaderImpl implements ExternalAttachmentLoader {
  ...
  public CompletableFuture<Void> fetchExternalAttachmentsAsLocal(Exam exam) {
    ...
    exam
      ...
      .map(
        sectionQuestion -> {
          if (sectionQuestion.getEssayAnswer() != null && sectionQuestion.getEssayAnswer().getAttachment() != null) {
            futures.add(
      ...
      .forEach(
        question ->
          futures.add(createFromExternalAttachment(question.getAttachment(), "question", question.getId().toString()))
      );
    ...
    return request
      ...
      .thenAcceptAsync(
        response -> {
          ...
          if (!file.exists()) {
            logger.warn("Could not find file {} for attachment id {}.", file.getAbsoluteFile(), attachment.getId());
            return;
          }
          final WSRequest updateRequest;
          try {
            updateRequest = wsClient.url(parseUrl("/api/attachments/%s", attachment.getExternalId()).toString());
          } catch (MalformedURLException e) {
              ...
              response
                ...
                .thenAccept(
                  ioResult -> {
                    ...
                    attachment.save();
                    logger.info("Saved attachment {} locally as # {}", attachment.getExternalId(), attachment.getId());
                  }
                );
            }
          );
