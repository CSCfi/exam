  ...
  private CompletionStage<Result> doCreateEnrolment(Long eid, ExamExecutionType.Type type, User user) {
    ...
    try {
      ...
      if (
        enrolments
          .stream()
          .anyMatch(e -> e.getExam().getRequiresUserAgentAuth() && e.getExaminationEventConfiguration() == null)
      ) {
        return wrapAsPromise(forbidden("sitnet_error_enrolment_exists"));
      }
      ...
      if (enrolmentsWithFutureReservations.size() > 1) {
        logger.error("Several enrolments with future reservations found for user {} and exam {}", user, exam.getId());
