  ...
  def addInspection(eid: Long, uid: Long): Action[AnyContent] = authenticated
    ...
    .andThen(audited) { request =>
      ...
      val result = for
        ...
        _ <-
          if !user.isAdminOrSupport && !exam.isOwnedOrCreatedBy(user) then Left(ExamInspectionError.AccessForbidden)
          else if examInspectionService.isInspectorOf(recipient, exam) then Left(ExamInspectionError.AlreadyInspector)
          else
        ...
        inspection = examInspectionService.addInspection(exam, recipient, user, request.attrs.get(ScalaAttrs.COMMENT))
      yield inspection

      result match
        case Right(inspection)                          => Ok(inspection.asJson)
        case Left(ExamInspectionError.AccessForbidden)  => Forbidden(ExamInspectionError.AccessForbidden.message)
        case Left(ExamInspectionError.AlreadyInspector) => Forbidden(ExamInspectionError.AlreadyInspector.message)
        case Left(ExamInspectionError.ExamNameMissing)  => BadRequest(ExamInspectionError.ExamNameMissing.message)
        case Left(ExamInspectionError.UserNotFound) | Left(ExamInspectionError.ExamNotFound) => NotFound
    ...
    Action
      ...
      .andThen(audited)(parse.json) { request =>
        ...
        examInspectionService.setOutcome(id, ready) match
          case Right(_)                                     => Ok
          case Left(ExamInspectionError.InspectionNotFound) => NotFound(ExamInspectionError.InspectionNotFound.message)
          case Left(_)                                      => NotFound
      }
