...
class MaintenancePeriodController @Inject() (
  ...
  private def toResult(error: MaintenancePeriodError): Result =
    error match
      ...
      case MaintenancePeriodError.BadPayload => BadRequest(MaintenancePeriodError.BadPayload.message)

  def listMaintenancePeriods: Action[AnyContent] =
    authenticated.andThen(authorized(Seq(Role.Name.STUDENT, Role.Name.TEACHER, Role.Name.ADMIN, Role.Name.SUPPORT))) {
      _ =>
  ...
  def createMaintenancePeriod: Action[JsValue] =
    authenticated(parse.json).andThen(authorized(Seq(Role.Name.ADMIN))).andThen(audited).async { request =>
