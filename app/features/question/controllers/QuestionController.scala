...
class QuestionController @Inject() (
  ...
  def getQuestions(
    ...
    authenticated.andThen(authorized(Seq(Role.Name.TEACHER, Role.Name.ADMIN, Role.Name.SUPPORT))) { request =>
      ...
      val (questions, pp) = questionService.getQuestions(user, examIds, courseIds, tagIds, sectionIds, ownerIds)
        ...
        request =>
          request.body.file("file") match
            ...
            case Some(filePart) =>
              ...
              Ok(play.api.libs.json.Json.obj("errorCount" -> errors.length, "successCount" -> successes.length))
      }
