...
describe('DiffInDaysPipe', () => {
    ...
    it('should return a span without text-danger for a positive diff', () => {
        const base = Date.now();
        const future = new Date(base + 3 * 24 * 60 * 60 * 1000).toISOString();
    ...
    it('should return a span with text-danger for a negative diff', () => {
        const base = Date.now();
        const past = new Date(base - 3 * 24 * 60 * 60 * 1000).toISOString();
        const now = new Date(base).toISOString();
    ...
    it('should use Math.floor for fractional days (positive)', () => {
        ...
        const base = Date.now();
        const from = new Date(base + 1.9 * 24 * 60 * 60 * 1000).toISOString();
    ...
    it('should use Math.floor for fractional days (negative)', () => {
        ...
        const base = Date.now();
        const from = new Date(base - 1.2 * 24 * 60 * 60 * 1000).toISOString();
        const to = new Date(base).toISOString();
        const result = pipe.transform(from, to);
